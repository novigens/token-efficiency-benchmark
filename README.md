...
**cost / 1M workflows / mo** = risk-adj `$/correct` × 1,000,000: the monthly bill to run a million of these workflows on that model, cleanup of wrong answers priced in. Same work, same month, Opus 4.8 costs **\$28.6K** and GPT-5.5 **\$933.5K**, a 33x spread. Read it as a make-or-buy line, not just a model ranking: these are workflows a back-office team could process and validate by hand (the Human row). So if hiring a team to run and check a million a month costs you *less* than the cheapest reliable model here (Opus at ~\$28.6K), hire the humans or go hybrid; LLM automation only pays when it is both reliable and cheaper than labor at your scale. Method, formula, gates, and full per-row counts: [`ANALYSIS.md`](benchmark_data/runs/20260706T222112Z_412022-paired/ANALYSIS.md).
